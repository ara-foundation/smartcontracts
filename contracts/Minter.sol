...
contract Minter is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize() initializer public {
        ...
        // ARA mints 1 Token to the deployer so that deployer can lint the smartcontracts
        rounds[1] = Round(12500000000000000000000, 24000000000000000, 1000000000000000000, 0, 0);

        // Second Round (Initial Set Up/Grant) = 0.1$/ARA, 87_500 Cap
        rounds[2] = Round(87500000000000000000000, 100000000000000000, 0, 0, 0);

        // Third Round (Angel Investment) = 0.18$/ARA, 1_900_000 Cap, 10,000$ min, 45,500$ max
        rounds[3] = Round(1900000000000000000000000, 180000000000000000, 0, 10000000000000000000000, 45500000000000000000000);

        // Fourth Round (A Series) = 0.2$/ARA, 18_000_000 Cap, 10,000$ min, 60,000$ max
        rounds[4] = Round(18000000000000000000000000, 200000000000000000, 0, 10000000000000000000000, 60000000000000000000000);

        // Fifth Round (B Series) = 0.22$/ARA, 12_500_000 Cap
        rounds[5] = Round(12500000000000000000000000, 220000000000000000, 0, 0, 0);

        // Sixth Round (Savings) = 0$/ARA, 10_000_000 Cap, 10,000$ min, 60,000$ max
        rounds[6] = Round(10000000000000000000000000, 0, 0, 10000000000000000000000, 60000000000000000000000);

        // Seventh Round (Public Exchanges) = 0.24$/ARA, 32_500_000 Cap
        rounds[7] = Round(32500000000000000000000000, 240000000000000000, 0, 0, 0);
    }
