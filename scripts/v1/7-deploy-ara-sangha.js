...
async function main() {
  ...
  const ownershipArgs = [projectId, leader, ownership, "Ara Test", "ARAt"]
  ...
  const maintainerArgs = [projectId, leader, ownership, "Ara Test Maintainer", "ARAtm"]
  contract = await Contract.deploy(...maintainerArgs)
  await contract.waitForDeployment()
  console.log(`Ara Maintainer deployed address ${await contract.getAddress()}`)

  const checkArgs = [projectId, leader, ownership, "Ara Test Check", "ARAtc"]
